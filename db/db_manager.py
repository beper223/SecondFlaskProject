...
class DatabaseManager:
    ...
    def get_movies(self,film,actor,category,year):
        query = """
                SELECT f.title, f.description, f.release_year, c.name AS category
                FROM film f
                ...
                AND (%s = '' OR a.first_name LIKE %s OR a.last_name LIKE %s)
                AND (%s = '' OR c.name LIKE %s)
                AND (%s = '' OR f.release_year = %s) \
                """
        params = [
            film, f"%{film}%",
